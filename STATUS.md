...
| model roster: qwen3.5-0.8b (default) · gemma-3-270m (ultra-lean) · any `.gguf` path | BUILT | picked 2026-08-11 after a landscape check; Apache-2.0 default matches the repo license. gemma-270m needed a named-template fix (its QAT GGUF ships none) and under greedy decoding recites the soul instead of answering — below the voice's floor, kept as the runs-anywhere artifact |
| soul — user-reshapeable identity (`~/.terminite/soul.md`, `voice soul --init`) | BUILT | shipped default embedded (assets/soul.md); code-appended ground rules are not overridable; safety is structural either way (no tools, no exec, no network at inference) |
| deterministic answers (greedy + repeat penalty) | BUILT | battery finding: sampling variance made the room answer pass twice then fail; post-fix, 3/3 identical correct runs. See lab/voice-boundaries-2026-08-11.md |
| Voice as a pane — chat module over `terminite ask` | BUILT | tools/voice-module (Data-kind, same wire as Config/Files); protocol-tested headless: 152 streamed frames, correct final render, roster matched room_who exactly (4 actors, 3 vendors). Each question is still one process-scoped ask — nothing resident. The pane itself: never seen by human eyes yet |
| the voice is *worth talking to* | NOT PROVEN | 12-question staged battery (lab/voice-boundaries-2026-08-11.md): grounded facts exact, refusals hold with redirects, Spanish free; open-ended explanations drift, injection tone erratic (nothing to leak — grounding is the user's own data). Staged ≠ lived-in; proof = Daniel using it |

...
impl Renderer {
    ...
    pub(super) fn apply_live_layout(&mut self) {
        // Every physical metric is the config's logical value × scale_factor
        // (HiDPI) — scale here too, or a focus-reload would snap padding /
        // gutters back to unscaled and break scaling on a 2x display.
        let s = self.scale_factor;
        let new_line_height =
            (self.font_size * LINE_H_RATIO * self.config.line_height).round();
        let new_pad = scale_padding(self.config.padding, s);
        let new_gutter_left = self.config.gutter_left * s;
        let new_gutter_gap = self.config.gutter_gap * s;
        let new_highlight_pad_x = self.config.highlight_pad_x * s;
        let new_highlight_pad_y = self.config.highlight_pad_y * s;
        let new_highlight_offset_y = self.config.highlight_offset_y * s;
        let new_tab_min_width = self.config.tab_min_width * s;
        let new_tab_max_width = self.config.tab_max_width * s;
        ...
        let colors_changed = new_bg != self.bg_color
            || new_tint != self.focus_tint
            || new_cursor != self.cursor_color
            || new_selection != self.selection_color;
        if !line_height_changed && !pad_or_gutter_changed && !colors_changed {
            return;
        }
    ...
    /// Re-derive physical font metrics from `base_font_size × scale_factor`,
    /// re-shape every tab's grid metrics, and drop the per-cell glyph cache so
    /// it re-warms at the new size. Does NOT relayout — the caller does.
    pub(super) fn apply_font_metrics(&mut self) {
        self.font_size = (self.base_font_size * self.scale_factor).round().max(1.0);
        self.cell_advance =
    ...
    /// (pre-HiDPI, logical) size; the physical raster size is re-derived by the

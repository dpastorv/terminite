...
pub trait Handler {
    ...
    /// OSC 7: the shell reports its working directory as a `file://` URI.
    /// (terminite fork.)
    fn set_current_directory(&mut self, _uri: String) {}

    /// OSC 133: a FinalTerm / iTerm2 shell-integration mark. `kind` is the
    /// mark letter — `A` prompt-start, `B` prompt-end (command input),
    /// `C` command-output start, `D` command finished — and `exit` carries
    ...
    T: Timeout,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // OSC 7: shell reports its working directory (file:// URI).
            // (terminite fork.)
            b"7" => {
                if params.len() >= 2 {
                    if let Ok(uri) = str::from_utf8(params[1]) {
                        self.handler.set_current_directory(uri.to_owned());
                        return;
                    }
                }
                unhandled(params);
            ...
            // OSC 133: FinalTerm / iTerm2 shell-integration marks.
            // (terminite fork.)
            b"133" => {
                let kind = params.get(1).and_then(|p| p.first()).copied();
                if let Some(kind) = kind {
                    let exit = params
                        .get(2)
                        .and_then(|p| str::from_utf8(p).ok())
                        .and_then(|s| s.parse::<i32>().ok());
                    self.handler.shell_integration(kind as char, exit);
                    return;
                }
                unhandled(params);
            },
